...
def try_methods():
    ...
    Then makes the SimpleTurtle move as follows (in the order listed):
      ...
      -- backward  100 units
    """
    podrick = rg.SimpleTurtle('turtle')
    podrick.pen = rg.Pen('brown', 5)
    podrick.forward(150)
    podrick.left(90)
    podrick.forward(50)
    podrick.backward(100)
    ####################################################################
...
def try_methods_and_functions():
    ...
    chip.pen = rg.Pen('blue',5)
    chip.backward(150)
    chip.speed = 1
    draw_many_squares(chip,2,100,30)
    chip.speed = 5
    chip.pen.color = 'red'
    draw_many_squares(chip,10,50,15)
    chip.speed = 100
    chip.pen.thickness = 35
    draw_many_squares(chip,8,300,60)
    chip.pen = rg.Pen('black',3)
    chip.backward(200)
    chip.draw_circle(30)
    chip.draw_square(50)
